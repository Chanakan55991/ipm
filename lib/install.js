  ...
  module.exports = Install = (function() {
    class Install extends Command {
      ...
      installNode(callback) {
        ...
        atomNodeGypPath = process.env.INKDROP_NODE_GYP_PATH || require.resolve('node-gyp/bin/node-gyp');
      ...
      installRegisteredPackage(metadata, options, callback) {
        ...
        return this.requestPackage(packageName, (error, pack) => {
          ...
          if (error != null) {
            ...
            if (!packageVersion) {
              this.logFailure();
              callback(`No available version compatible with the installed Inkdrop version: ${this.installedAtomVersion}`);
              return;
            }
      ...
      getLatestCompatibleVersion(pack) {
        ...
        for (version in ref1) {
          ...
          engine = (ref2 = (ref3 = metadata.engines) != null ? ref3.inkdrop : void 0) != null ? ref2 : '*';
        ...
        installPackage = (name, nextInstallStep) => {
          ...
          if (gitPackageInfo || name.indexOf('file://') === 0) {
            ...
            return this.isBundledPackage(name, (isBundledPackage) => {
              if (isBundledPackage) {
                console.error(`The ${name} package is bundled with Inkdrop and should not be explicitly installed.\nYou can run \`ipm uninstall ${name}\` to uninstall it and then the version bundled\nwith Inkdrop will be used.`.yellow);
              }
